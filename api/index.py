...
import json, sqlite3, uuid, bcrypt, os, re, time
from datetime import datetime, date, timedelta
from pathlib import Path

app = FastAPI()

ALLOWED = os.environ.get("ALLOWED_ORIGINS", "https://follow-up-os-production.up.railway.app").split(",")
app.add_middleware(CORSMiddleware, allow_origins=ALLOWED, allow_methods=["GET","POST","PUT"], allow_headers=["*"])
...
async def session_middleware(request: Request, call_next):
    ...
    if not request.cookies.get("session_id") and not request.cookies.get("token"):
        response.set_cookie("session_id", str(uuid.uuid4()), httponly=True, secure=True, samesite="lax")
    return response
...
def init_db():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS push_subscriptions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, session_id TEXT, subscription_json TEXT, created TEXT)")
    conn.commit()
    return conn
...
<meta name="apple-mobile-web-app-title" content="Follow-Up OS"><link rel="manifest" href="/manifest.json"><title>Follow-Up OS</title>
...
<div id='saveBanner' class='save-banner' style='display:none'><span>⚠️ Leads lost when tab closes.</span><input type='email' id='bannerEmail' placeholder='Email'><input type='password' id='bannerPassword' placeholder='create Password (8+ chars, Aa1)'><button class='btn' onclick='saveAccount()'>Save</button></div>
...
<div id='installFloat' style='display:none;position:fixed;bottom:20px;left:20px;right:20px;background:var(--blue);color:#fff;padding:14px 20px;border-radius:14px;text-align:center;font-weight:600;font-size:.9rem;cursor:pointer;z-index:100;box-shadow:0 8px 30px rgba(59,130,246,0.4)' onclick='installApp()'>📲 Tap to add to Home Screen — get daily reminders</div>
<script>
var current='add',isLoggedIn=false,deferredPrompt=null;
window.addEventListener('beforeinstallprompt',function(e){e.preventDefault();deferredPrompt=e;document.getElementById('installFloat').style.display='block'});
function installApp(){if(deferredPrompt){deferredPrompt.prompt();deferredPrompt.userChoice.then(function(r){if(r.outcome==='accepted'){document.getElementById('installFloat').style.display='none';subscribePush()}})}}
function subscribePush(){if('serviceWorker' in navigator && 'PushManager' in window){navigator.serviceWorker.ready.then(function(reg){reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlB64('BEl62iI1f5qEd4oEP0kMfv3hOZ0xqPh4JqX3kPqNt4Hq4d0c5t6h8k3m2p1q0r5s')}).then(function(s){fetch('/api/push/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subscription:s})})})})}}
function urlB64(b){var p='='.repeat((4-b.length%4)%4);var r=atob((b+p).replace(/-/g,'+').replace(/_/g,'/'));var o=new Uint8Array(r.length);for(var i=0;i<r.length;i++)o[i]=r.charCodeAt(i);return o}
...
async def manifest():
    return JSONResponse({"name":"Follow-Up OS","short_name":"Follow-Up","start_url":"/","display":"standalone","background_color":"#06060e","theme_color":"#3b82f6","icons":[{"src":"/icon-192.png","sizes":"192x192","type":"image/png"},{"src":"/icon-512.png","sizes":"512x512","type":"image/png"}]})

@app.get("/sw.js")
async def service_worker():
    return Response(content="""self.addEventListener('push',function(e){var d=e.data.json();e.waitUntil(self.registration.showNotification(d.title,{body:d.body,icon:'/icon-192.png',badge:'/icon-192.png',tag:'followup',renotify:true}))});self.addEventListener('notificationclick',function(e){e.notification.close();e.waitUntil(clients.openWindow('/'))});""",media_type="application/javascript")

@app.post("/api/push/subscribe")
async def push_subscribe(request: Request):
    if not rate_limit(request.cookies.get("session_id","0"), 10, 60): return JSONResponse({"error":"Too many requests"},status_code=429)
    data = await request.json()
    user = fetch_user(request)
    uid = user["id"] if user else 0
    sid = request.cookies.get("session_id","0")
    conn = sqlite3.connect(str(DB))
    conn.execute("INSERT INTO push_subscriptions (user_id,session_id,subscription_json,created) VALUES (?,?,?,?)",(uid,sid,json.dumps(data.get("subscription",{})),datetime.now().isoformat()))
    conn.commit();conn.close()
    return {"ok":True}
...
async def signup(request: Request):
    if not rate_limit("signup_"+request.client.host, 10, 300): return JSONResponse({"error":"Too many attempts"},status_code=429)
    data = await request.json()
    email = data.get("email","").strip().lower()
    password = data.get("password","").strip()
    if not email or len(password) < 8: return JSONResponse({"error":"Password must be 8+ characters"},status_code=400)
    if not re.search(r"[a-z]", password) or not re.search(r"[A-Z]", password) or not re.search(r"[0-9]", password): return JSONResponse({"error":"Password needs upper, lower, and number"},status_code=400)
    if not re.match(r"^[^@]+@[^@]+\.[^@]+$", email): return JSONResponse({"error":"Invalid email"},status_code=400)
    conn = sqlite3.connect(str(DB))
    try:
        ...
        resp.set_cookie("token",token,httponly=True,secure=True,samesite="lax",max_age=86400*365)
        return resp
...
async def login(request: Request):
    if not rate_limit("login_"+request.client.host, 10, 300): return JSONResponse({"error":"Too many attempts"},status_code=429)
    data = await request.json()
    email = data.get("email","").strip().lower()
    password = data.get("password","").strip()
    conn = sqlite3.connect(str(DB)); conn.row_factory = sqlite3.Row
...
async def add_lead(request: Request):
    if not rate_limit(request.cookies.get("session_id","0"),60,60): return JSONResponse({"error":"Too many leads"},status_code=429)
    user = fetch_user(request); uid = user["id"] if user else 0
    sid = request.cookies.get("session_id",str(uuid.uuid4()))
    data = await request.json()
    if not data.get("name","").strip(): return JSONResponse({"error":"Name required"},status_code=400)
    if len(data.get("name",""))>100: return JSONResponse({"error":"Name too long"},status_code=400)
    try:
        dv = int(data.get("deal_value",0))
        if dv<0: raise ValueError
    except: return JSONResponse({"error":"Invalid deal value"},status_code=400)
    now = datetime.now().isoformat()
    conn = sqlite3.connect(str(DB))
    try:
        conn.execute("INSERT INTO leads (user_id,session_id,name,source,note,deal_value,status,follow_up_date,created,updated) VALUES (?,?,?,?,?,?,?,?,?,?)",(uid,sid,data.get("name",""),data.get("source",""),data.get("note",""),dv,data.get("status","warm"),data.get("follow_up_date",date.today().isoformat()),now,now))
        conn.commit()
    except sqlite3.Error:
        conn.close(); return JSONResponse({"error":"Database error"},status_code=500)
    conn.close()
    resp = JSONResponse({"ok":True})
    if not request.cookies.get("session_id"): resp.set_cookie("session_id",sid,httponly=True,secure=True,samesite="lax")
    return resp
...
async def update_lead(lead_id:int, request: Request):
    ...
    conn = sqlite3.connect(str(DB))
    try:
        if uid>0: conn.execute("UPDATE leads SET status=?,updated=? WHERE id=? AND user_id=?",(data.get("status","warm"),now,lead_id,uid))
        else: conn.execute("UPDATE leads SET status=?,updated=? WHERE id=? AND session_id=?",(data.get("status","warm"),now,lead_id,sid))
        if "follow_up_date" in data:
            if uid>0: conn.execute("UPDATE leads SET follow_up_date=?,updated=? WHERE id=? AND user_id=?",(data["follow_up_date"],now,lead_id,uid))
            else: conn.execute("UPDATE leads SET follow_up_date=?,updated=? WHERE id=? AND session_id=?",(data["follow_up_date"],now,lead_id,sid))
        conn.commit()
    except sqlite3.Error:
        conn.close(); return JSONResponse({"error":"Database error"},status_code=500)
    conn.close()
...
async def dashboard_login(request: Request):
    ...
    if data.get("password") == ADMIN_PASS:
        ...
        resp.set_cookie("admin_token",ADMIN_PASS,httponly=True,secure=True,samesite="lax")
        return resp
