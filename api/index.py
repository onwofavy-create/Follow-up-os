...
import json, os, io, uuid, bcrypt, psycopg2
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            token TEXT, streak INTEGER DEFAULT 0, last_streak_date TEXT,
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS leads (
            ...
            session_id TEXT DEFAULT '0', name TEXT NOT NULL, source TEXT, note TEXT,
            deal_value INTEGER DEFAULT 0, status TEXT DEFAULT 'warm',
            follow_up_date TEXT, last_contacted TEXT, nudge_enabled INTEGER DEFAULT 0,
            created TEXT, updated TEXT
        )
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS history (
            id SERIAL PRIMARY KEY, lead_id INTEGER, user_id INTEGER,
            action TEXT, note TEXT, created TEXT
        )
    """)
...
def get_user(request: Request):
    ...
    conn.close()
    if row:
        cols = ["id","email","password","token","streak","last_streak_date","created","last_login"]
...
APP_HTML = r"""<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1.0'><title>Follow-Up OS</title><style>:root{--bg:#06060e;--surface:#0c0c1a;--border:#1a1a35;--text:#e0e0e8;--muted:#7a7a90;--blue:#3b82f6;--blue-glow:#3b82f640;--orange:#ff6b35;--success:#22c55e;--warn:#f59e0b;--danger:#ef4444}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;padding:24px}.container{max-width:960px;margin:0 auto}.topbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:28px;flex-wrap:wrap;gap:12px}.logo-text{font-size:1.4rem;font-weight:800;color:#fff;letter-spacing:-0.5px}.logo-text span{background:linear-gradient(135deg,var(--blue),var(--orange));-webkit-background-clip:text;-webkit-text-fill-color:transparent}.topbar-right{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.streak{background:var(--surface);border:1px solid var(--blue);color:var(--blue);padding:6px 14px;border-radius:20px;font-size:.75rem;font-weight:600}.user-email{color:var(--muted);font-size:.8rem}.logout{color:var(--muted);text-decoration:none;font-size:.8rem;padding:8px 16px;background:var(--surface);border:1px solid var(--border);border-radius:20px;transition:.2s}.logout:hover{border-color:var(--danger);color:var(--danger)}.save-banner{background:var(--surface);border:1px solid var(--warn);border-radius:16px;padding:20px 24px;margin-bottom:28px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:center}.save-banner span{color:var(--warn);font-weight:600;font-size:.85rem}.save-banner input{background:var(--bg);border:1px solid var(--border);color:#fff;padding:12px 16px;border-radius:12px;font-size:.85rem;min-width:180px}.save-banner .btn-sm{background:var(--blue);color:#fff;border:none;padding:12px 20px;border-radius:12px;font-size:.85rem;font-weight:600;cursor:pointer}.summary{display:flex;gap:16px;margin-bottom:28px;flex-wrap:wrap}.summary-card{flex:1;min-width:140px;background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:24px;text-align:center;transition:.2s}.summary-card:hover{border-color:var(--blue)}.summary-card .num{font-size:2.4rem;font-weight:800}.summary-card .lbl{color:var(--muted);font-size:.7rem;text-transform:uppercase;letter-spacing:1px;margin-top:4px}.today{border-color:#3b82f640}.today .num{color:var(--blue)}.overdue{border-color:#ef444440}.overdue .num{color:var(--danger)}.active{border-color:#22c55e40}.active .num{color:var(--success)}.tabs{display:flex;gap:6px;margin-bottom:28px;flex-wrap:wrap}.tab{flex:1;min-width:100px;text-align:center;padding:14px 12px;border-radius:14px;cursor:pointer;font-weight:600;font-size:.8rem;color:var(--muted);background:var(--surface);border:1px solid var(--border);transition:.2s}.tab.active{background:var(--blue);color:#fff;border-color:var(--blue)}.tab .count{background:#fff;color:var(--blue);padding:2px 8px;border-radius:10px;font-size:.65rem;margin-left:6px;font-weight:800}.panel{display:none}.panel.active{display:block}.card{background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:32px;margin-bottom:20px}.card h3{color:var(--muted);font-size:.7rem;text-transform:uppercase;letter-spacing:2px;margin-bottom:20px}label{display:block;color:var(--muted);font-size:.65rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px}input,textarea,select{width:100%;background:var(--bg);border:1px solid var(--border);color:#fff;padding:14px 16px;border-radius:14px;font-size:.9rem;font-family:inherit;outline:none;margin-bottom:16px;transition:.2s}textarea{resize:vertical;min-height:60px}input:focus,textarea:focus,select:focus{border-color:var(--blue);box-shadow:0 0 0 3px var(--blue-glow)}.btn{background:var(--blue);color:#fff;border:none;padding:15px;border-radius:14px;font-weight:700;font-size:.9rem;cursor:pointer;width:100%;transition:.2s}.btn:hover{opacity:.9}.btn-sm{background:var(--blue);color:#fff;border:none;padding:10px 16px;border-radius:12px;font-size:.75rem;cursor:pointer;font-weight:600;transition:.2s}.btn-sm:hover{opacity:.9}.btn-orange{background:var(--orange)}.btn-success{background:var(--success)}.btn-danger{background:var(--danger)}.btn-warn{background:var(--warn);color:#000}.lead-row{background:var(--bg);border:1px solid var(--border);border-radius:16px;padding:20px;margin:10px 0;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:14px;transition:.2s}.lead-row:hover{border-color:var(--blue)}.lead-row .info{flex:1;min-width:200px}.lead-row .name{color:#fff;font-weight:600;font-size:1rem}.lead-row .detail{color:var(--muted);font-size:.75rem;margin-top:4px}.lead-row .actions{display:flex;gap:8px;flex-wrap:wrap}.badge{display:inline-block;padding:3px 10px;border-radius:12px;font-size:.65rem;font-weight:700}.badge.hot{background:#22c55e20;color:var(--success)}.badge.warm{background:#f59e0b20;color:var(--warn)}.badge.cold{background:#ef444420;color:var(--danger)}.badge.lost{background:#88888820;color:var(--muted)}.badge.closed{background:#22c55e40;color:#fff}.overdue-row{border-left:5px solid var(--danger)}.today-row{border-left:5px solid var(--blue)}.empty{text-align:center;color:var(--muted);padding:60px 20px;font-size:.9rem}.spinner{display:inline-block;width:16px;height:16px;border:2px solid #fff3;border-top-color:#fff;border-radius:50%;animation:spin .6s linear infinite;margin-right:8px;vertical-align:middle}@keyframes spin{to{transform:rotate(360deg)}}.modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);z-index:999;align-items:center;justify-content:center}.modal.active{display:flex}.modal-content{background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:36px;max-width:440px;text-align:center}.modal-content h2{color:var(--success);margin-bottom:12px}.modal-content textarea{min-height:80px;margin-top:12px}.share-btn{background:var(--success)!important;margin-top:8px}.nudge-btn{background:transparent;border:1px solid var(--border);color:var(--muted);padding:6px 12px;border-radius:10px;font-size:.7rem;cursor:pointer;transition:.2s}.nudge-btn.active{background:var(--warn);color:#000;border-color:var(--warn)}@media(max-width:640px){.topbar{flex-direction:column;text-align:center}.summary{flex-direction:column}.tabs{flex-direction:column}.lead-row{flex-direction:column;text-align:center}.lead-row .actions{justify-content:center}.modal-content{margin:0 16px}}</style></head><body><div class='container'><div class='topbar'><div class='logo-text'><span>Follow-Up</span> OS</div><div class='topbar-right'><span id='streakBadge' class='streak' style='display:none'></span><span class='user-email' id='userEmail'></span><a href='/api/logout' class='logout' id='logoutLink' style='display:none'>Logout</a></div></div><div id='saveBanner' class='save-banner' style='display:none'><span>⚠️ Your leads will be lost when you close this tab.</span><input type='email' id='bannerEmail' placeholder='Your email'><input type='password' id='bannerPassword' placeholder='Create password'><button class='btn-sm' onclick='saveAccount()'>Save My Leads</button><span id='bannerError' style='color:#ef4444;font-size:.75rem'></span></div><div class='summary' id='summary'></div><div class='tabs'><div class='tab active' onclick='switchTab(\"add\")'>➕ Add Lead</div><div class='tab' onclick='switchTab(\"today\")'>🔥 Today<span class='count' id='todayCount'>0</span></div><div class='tab' onclick='switchTab(\"active\")'>📊 Active<span class='count' id='activeCount'>0</span></div><div class='tab' onclick='switchTab(\"lost\")'>📁 Closed/Lost</div></div><div id='add' class='panel active'><div class='card'><h3>New Lead</h3><label>Name</label><input id='l_name' placeholder='Client or business name'><label>Source</label><select id='l_source'><option>Instagram</option><option>WhatsApp</option><option>Referral</option><option>Other</option></select><label>Note</label><textarea id='l_note' placeholder='What do they need? Any context?'></textarea><label>Deal Value (₦)</label><input id='l_value' placeholder='e.g. 150000' type='number'><label>Status</label><select id='l_status'><option value='hot'>🔥 Hot</option><option value='warm' selected>🟡 Warm</option><option value='cold'>🔴 Cold</option></select><label>Follow-up Date</label><input id='l_date' type='date'><button class='btn' id='addBtn' onclick='addLead()'>Save Lead</button></div></div><div id='today' class='panel'><div class='card'><h3>Today's Follow-ups</h3><div id='todayList'><div class='empty'>Loading...</div></div></div></div><div id='active' class='panel'><div class='card'><h3>All Active Leads</h3><div id='activeList'><div class='empty'>Loading...</div></div></div></div><div id='lost' class='panel'><div class='card'><h3>Closed & Lost</h3><div id='lostList'><div class='empty'>Loading...</div></div></div></div></div><div class='modal' id='winModal'><div class='modal-content'><h2>🎉 Deal Closed!</h2><p id='winAmount' style='color:var(--text);margin-bottom:12px'></p><textarea id='winMessage' readonly></textarea><button class='btn share-btn' onclick='copyWin()'>📋 Copy & Share</button><button class='btn-sm' style='margin-top:10px;background:var(--muted)' onclick='closeModal()'>Close</button></div></div><script>let current='add',isLoggedIn=false;fetch('/api/me').then(r=>r.json()).then(d=>{if(d.email){isLoggedIn=true;document.getElementById('userEmail').textContent=d.email;document.getElementById('logoutLink').style.display='inline';document.getElementById('saveBanner').style.display='none';if(d.streak>1){document.getElementById('streakBadge').textContent='🔥 '+d.streak+' day streak';document.getElementById('streakBadge').style.display='inline'}}else{document.getElementById('saveBanner').style.display='flex'}loadSummary()});function switchTab(t){current=t;document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.panel').forEach(x=>x.classList.remove('active'));document.getElementById(t).classList.add('active');document.querySelector('.tab:nth-child('+(t==='add'?1:t==='today'?2:t==='active'?3:4)+')').classList.add('active');if(t==='today')loadToday();if(t==='active')loadActive();if(t==='lost')loadLost();loadSummary()}function loadSummary(){fetch('/api/summary').then(r=>r.json()).then(d=>{document.getElementById('summary').innerHTML='<div class=\"summary-card today\"><div class=\"num\">'+d.today+'</div><div class=\"lbl\">Due Today</div></div><div class=\"summary-card overdue\"><div class=\"num\">'+d.overdue+'</div><div class=\"lbl\">Overdue</div></div><div class=\"summary-card active\"><div class=\"num\">'+d.active+'</div><div class=\"lbl\">Active Pipeline</div></div>';document.getElementById('todayCount').textContent=d.today;document.getElementById('activeCount').textContent=d.active})}function renderLeads(leads,container){let h='';if(!leads.length)h='<div class=\"empty\">Nothing here. Add your first lead above.</div>';leads.forEach(l=>{let cls='';if(l.follow_up_date<new Date().toISOString().split('T')[0])cls='overdue-row';else if(l.follow_up_date===new Date().toISOString().split('T')[0])cls='today-row';h+='<div class=\"lead-row '+cls+'\"><div class=\"info\"><div class=\"name\">'+l.name+' <span class=\"badge '+l.status+'\">'+l.status.toUpperCase()+'</span></div><div class=\"detail\">₦'+(l.deal_value||0).toLocaleString()+' • '+l.follow_up_date+' • '+l.note+'</div></div><div class=\"actions\">';if(l.status!=='lost'&&l.status!=='closed'){h+='<button class=\"nudge-btn'+(l.nudge_enabled?' active':'')+'\" onclick=\"toggleNudge('+l.id+')\">🔔 Nudge</button>';h+='<button class=\"btn-sm btn-success\" onclick=\"updateStatus('+l.id+',\\'closed\\')\">✅ Won</button>';h+='<button class=\"btn-sm btn-danger\" onclick=\"updateStatus('+l.id+',\\'lost\\')\">❌ Lost</button>';h+='<button class=\"btn-sm btn-warn\" onclick=\"followedUp('+l.id+')\">📞 Followed Up</button>'}h+='<button class=\"btn-sm\" onclick=\"downloadReminder('+l.id+')\">📅 Remind</button></div></div>'});document.getElementById(container).innerHTML=h}function loadToday(){fetch('/api/leads/today').then(r=>r.json()).then(d=>renderLeads(d,'todayList'))}function loadActive(){fetch('/api/leads/active').then(r=>r.json()).then(d=>renderLeads(d,'activeList'))}function loadLost(){fetch('/api/leads/lost').then(r=>r.json()).then(d=>renderLeads(d,'lostList'))}async function saveAccount(){const email=document.getElementById('bannerEmail').value;const password=document.getElementById('bannerPassword').value;if(!email||password.length<6){document.getElementById('bannerError').textContent='Enter a valid email and password (6+ characters)';return}const r=await fetch('/api/signup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password})});if(r.ok)location.reload();else{const d=await r.json();document.getElementById('bannerError').textContent=d.error}}async function addLead(){const btn=document.getElementById('addBtn');const orig=btn.innerHTML;btn.disabled=true;btn.innerHTML='<span class=\"spinner\"></span>';const r=await fetch('/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.getElementById('l_name').value,source:document.getElementById('l_source').value,note:document.getElementById('l_note').value,deal_value:document.getElementById('l_value').value,status:document.getElementById('l_status').value,follow_up_date:document.getElementById('l_date').value})});document.getElementById('l_name').value='';document.getElementById('l_note').value='';document.getElementById('l_value').value='';btn.disabled=false;btn.innerHTML=orig;loadSummary();if(!isLoggedIn)document.getElementById('saveBanner').style.display='flex'}async function updateStatus(id,status){await fetch('/api/leads/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})});if(status==='closed'){document.getElementById('winAmount').textContent='Deal closed successfully!';document.getElementById('winMessage').value='Just closed another deal. Staying organized with Follow-Up OS keeps me on track.';document.getElementById('winModal').classList.add('active')}if(current==='today')loadToday();else if(current==='active')loadActive();else loadLost();loadSummary()}function closeModal(){document.getElementById('winModal').classList.remove('active')}function copyWin(){navigator.clipboard.writeText(document.getElementById('winMessage').value);alert('Copied! Share your win.')}async function followedUp(id){let d=new Date();d.setDate(d.getDate()+3);await fetch('/api/leads/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({follow_up_date:d.toISOString().split('T')[0],last_contacted:new Date().toISOString()})});if(current==='today')loadToday();else loadActive();loadSummary()}async function toggleNudge(id){await fetch('/api/leads/'+id+'/nudge',{method:'POST'});if(current==='today')loadToday();else loadActive()}function downloadReminder(id){window.open('/api/leads/'+id+'/reminder','_blank')}document.getElementById('l_date').value=new Date().toISOString().split('T')[0];loadSummary();</script></body></html>"""

COS_HTML = r"""<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1.0'><title>Cost of Silence</title><style>:root{--bg:#06060e;--surface:#0c0c1a;--border:#1a1a35;--text:#e0e0e8;--muted:#7a7a90;--blue:#3b82f6;--orange:#ff6b35;--danger:#ef4444}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px}.container{max-width:640px}.logo{font-size:1.5rem;font-weight:800;color:#fff;margin-bottom:40px;letter-spacing:-0.5px}.logo span{background:linear-gradient(135deg,var(--blue),var(--orange));-webkit-background-clip:text;-webkit-text-fill-color:transparent}.number{font-size:clamp(3rem,8vw,5rem);font-weight:800;color:var(--danger);line-height:1.1}.label{color:var(--muted);font-size:1rem;margin-bottom:36px}.stats{display:flex;gap:16px;justify-content:center;flex-wrap:wrap;margin-bottom:36px}.stat{background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:24px 28px;min-width:150px}.stat .val{font-size:1.8rem;font-weight:700;color:var(--blue)}.stat .lbl{color:var(--muted);font-size:.7rem;text-transform:uppercase;letter-spacing:1px;margin-top:4px}.btn{display:inline-block;background:var(--blue);color:#fff;text-decoration:none;padding:16px 36px;border-radius:14px;font-weight:700;font-size:.95rem;transition:.2s}.btn:hover{opacity:.9;box-shadow:0 8px 25px #3b82f640}</style></head><body><div class='container'><div class='logo'><span>Follow-Up</span> OS</div><div class='number' id='total'>...</div><div class='label'>in unclosed deals dying in DMs right now.</div><div class='stats'><div class='stat'><div class='val' id='daily'>...</div><div class='lbl'>Estimated Daily Loss</div></div><div class='stat'><div class='val' id='count'>...</div><div class='lbl'>Overdue Deals</div></div></div><a href='/app' class='btn'>Stop Losing Money — It's Free</a></div><script>fetch('/api/public/cost-of-silence').then(r=>r.json()).then(d=>{document.getElementById('total').textContent='₦'+d.total_at_risk.toLocaleString();document.getElementById('daily').textContent='₦'+d.daily_loss_estimate.toLocaleString();document.getElementById('count').textContent=d.overdue_deals})</script></body></html>"""
...
async def resurrect_api(request: Request):
    ...
    context_line = f"I was reviewing our conversation about {context}" if context else "I was going through old messages"
    urgency = "I realize I should have followed up sooner" if int(days) > 5 else "I wanted to follow up"
    message = f"""{greeting},\n\n{urgency}. {context_line} and {last_msg[:80] if last_msg else 'our last chat'} came to mind.\n\nNo pressure at all — if you're still interested, I'd love to pick this back up. If things have changed, no worries either. Just wanted to check in.\n\n{'- ' + name if name else '- [Your Name]'}"""
...
async def toggle_nudge(lead_id: int, request: Request):
    ...
    if rows: run_query("UPDATE leads SET nudge_enabled=%s, updated=%s WHERE id=%s AND user_id=%s", (0 if rows[0]["nudge_enabled"] else 1, datetime.now().isoformat(), lead_id, uid))
    return {"ok": True}
...
async def dashboard_page(request: Request):
    if request.cookies.get("admin_token") != ADMIN_PASS:
        return """<html><body style="background:#06060e;color:#fff;font-family:system-ui;display:flex;align-items:center;justify-content:center;height:100vh"><form method="post" action="/dashboard/login"><input name="password" type="password" placeholder="Admin password" style="padding:14px;background:#0c0c1a;border:1px solid #1a1a35;color:#fff;border-radius:14px;margin-bottom:12px"><button style="padding:14px 24px;background:#3b82f6;color:#fff;border:none;border-radius:14px;cursor:pointer;font-weight:600">Access</button></form></body></html>"""
    rows = run_query("SELECT id, email, created, last_login, streak FROM users ORDER BY last_login DESC")
    html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Dashboard</title><style>body{font-family:system-ui;background:#06060e;color:#d4d4d8;padding:40px}h1{color:#3b82f6}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{padding:14px;text-align:left;border-bottom:1px solid #1a1a35}th{color:#3b82f6}.num{color:#22c55e;font-weight:700}</style></head><body><h1>Dashboard</h1><table><tr><th>Email</th><th>Leads</th><th>Signed Up</th><th>Last Login</th><th>Streak</th></tr>"
    for u in (rows or []):
